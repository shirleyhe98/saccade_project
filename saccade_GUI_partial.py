# saccade_GUI.py
# This program constructs a GUI to display part of the original saccade data, denoised 
# saccade data and detected saccades.
...
from algorithms.CGTV_algo_partial_data import CGTV_algo_partial_data
import numpy as np
import pandas as pd
...
def update_displayed_data():
    ...
    """
    if patient_group == 1:
        # patient group is concussion
        patient_label.configure(text="%s group: patient number: %d" % ('Concussion', patient_num))
    else:
        # patient group is control
        patient_label.configure(text="%s group: patient number: %d" % ('Control', patient_num))

def load_antisaccade_data_partial(patient_group, patient_num, Fs):
    """
    This function loads the partial antisaccade data and find the saccade detection array and total
    ...
    # Denoise velocity
    denoised_vel = denoise_vel(horizontal_saccade, Fs, movavg=0.02)

    # Obtain window length from the user
    ...
    start_index, win_time, win_signal, denoised_partial_signal, partial_detection, num_sacs = CGTV_algo_partial_data(Fs, horizontal_saccade, saccade_time_data, denoised_vel, window_len)
    
    # Set window start and end time
    start_time = df["time"][start_index]
    end_time = start_time + window_len
    
    # Obtain target partial samples
    target_partial_samples = df[(df["time"] >= start_time) & (df["time"] < end_time)]
    win_target_samples = np.array(target_partial_samples["target position"])

    # Data that needed to be displayed
    data_plt_map = {}
    data_plt_map['partial detection'] = {'visible_var': partial_detection_line, 'params': [win_time, partial_detection, 'g']}
    data_plt_map['partial original singal'] = {'visible_var': partial_original_line, 'params': [win_time, win_signal, 'b']}
    data_plt_map['partial denoised signal'] = {'visible_var': partial_denoised_line, 'params': [win_time, denoised_partial_signal, 'r']}
    data_plt_map['partial target signal'] = {'visible_var': partial_target_line, 'params': [win_time, win_target_samples, 'orange']}
...
def set_GUI_plot(figure):
    ...
    axis0.set_title("CGTV: %d saccades detected in total" % num_sacs, fontsize=16)

    # plot partial detection, partial original signal, partial target signal, and partial denoised signal
...
def update_patient():
    ...
    load_antisaccade_data_partial(patient_group, patient_num, Fs)
...
partial_detection_line = tk.IntVar(value=1)
partial_original_line = tk.IntVar(value=1)
partial_denoised_line = tk.IntVar(value=1)
partial_target_line = tk.IntVar(value=1)
load_antisaccade_data_partial(patient_group, patient_num, Fs)
